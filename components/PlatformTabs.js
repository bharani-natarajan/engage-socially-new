...
export default function PlatformTabs({ platform, onChange }) {
  return (
    <div className="flex gap-1 border-b border-lord-border mb-6">
      {TABS.map((tab) => {
        const isActive = platform === tab.value;
        return (
          <button
            key={tab.value}
            ...
            className={`flex items-center gap-2 px-4 py-2.5 text-[13px] font-semibold border-b-2 -mb-px transition-colors ${
              isActive
                ? tab.activeColor
                : 'border-transparent text-lord-text-muted hover:text-lord-text-main'
            }`}
          >
            {tab.icon}
            {tab.label}
