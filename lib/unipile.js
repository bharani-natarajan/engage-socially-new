...
export function normalizeComment(comment) {
  const authorInfo = comment.author_info ?? {};
  const name = authorInfo.name ?? null;
  const publicId = authorInfo.public_identifier ?? null;
  const providerId = authorInfo.provider_id ?? null;
  const displayName = name ?? publicId ?? providerId ?? 'LinkedIn Member';
