...
async function req(path, { method = 'GET', body, form } = {}) {
  ...
  if (!res.ok) throw new Error(data.message || data.error || `Unipile ${res.status}`);
  return data;
}

// ── Auth ──────────────────────────────────────────────────────────────────────

export async function getHostedAuthLink(successUrl, failureUrl) {
  return req('/hosted/accounts/link', {
    method: 'POST',
    body: {
      type: 'LINKEDIN',
      api_url: `https://${process.env.UNIPILE_DSN}/api/v1`,
...
export async function getPosts(accountId) {
  return req(`/users/posts?account_id=${encodeURIComponent(accountId)}&limit=20`);
}
