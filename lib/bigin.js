...
const API_BASE  = process.env.ZOHO_API_URL ?? 'https://www.zohoapis.com';
...
export function buildAuthUrl(redirectUri, state) {
  const params = new URLSearchParams({
    ...
    scope:         'ZohoBigin.modules.contacts.CREATE,ZohoBigin.modules.contacts.READ',
...
export async function createContact(accessToken, lead) {
  ...
  const body = {
    data: [{
      ...
      ...(lead.username ? { Twitter: `@${lead.username}` } : {}),
      Tag: [
        { name: platformLabel },
  ...
  return req(accessToken, '/Contacts', { method: 'POST', body: JSON.stringify(body) });
}
