  ...
  if (!token || !userId) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center px-6 py-20">
        <p className="text-gray-500 mb-4">Connect Instagram to view post details.</p>
        <a
          href="/api/auth/instagram"
          className="px-5 py-2.5 bg-gradient-to-r from-violet-600 to-pink-600 text-white rounded-xl text-sm font-medium hover:opacity-90"
        >
          Connect Instagram
        </a>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-xl bg-white border border-gray-200 p-5">
            ...
            {insights.video_views !== undefined && (
              <MetricRow label="Video Views" value={insights.video_views} icon="▶" />
            )}
          </div>
            ...
            <div className="space-y-2.5">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-700 capitalize">
                  {post.media_type?.replace('_', ' ').toLowerCase() ?? '—'}
                </span>
              </div>
              <div className="flex justify-between text-sm gap-4">
                <span className="text-gray-500 flex-shrink-0">Published</span>
                <span className="text-gray-700 text-right text-xs leading-relaxed">
                  {formattedDate}
                </span>
              </div>
            </div>
            {post.permalink && (
              <div className="mt-4 pt-4 border-t border-gray-100">
                <a
                  ...
                  className="flex items-center justify-center gap-2 py-2.5 rounded-lg border border-gray-200 text-sm text-gray-500 hover:text-violet-600 hover:border-violet-300 transition-colors"
                >
                  View on Instagram
