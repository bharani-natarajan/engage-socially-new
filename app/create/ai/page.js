      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
          <div>
            ...
            {preview ? (
              <div className="relative rounded-2xl overflow-hidden border border-gray-100 bg-gray-50 aspect-square">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={preview} alt="Product" className="w-full h-full object-contain" />
                <button onClick={() => { setFile(null); setImages([]); }} className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white shadow-md flex items-center justify-center text-gray-500 hover:text-red-500 transition-colors">
              ...
              <label
                ...
                className={`aspect-square rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-3 cursor-pointer transition-colors ${dragging ? 'border-green-400 bg-green-50' : 'border-gray-200 bg-gray-50 hover:border-green-400 hover:bg-green-50/40'}`}
              >
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-800 mb-2">Scene Description</label>
            <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="e.g. a modern kitchen with marble countertops and soft natural light" rows={3} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-100 focus:border-green-400 transition-colors resize-none" />
          </div>

          <div>
            <p className="text-xs text-gray-400 mb-2 font-medium">Quick styles</p>
            <div className="flex flex-wrap gap-2">
              {STYLE_PROMPTS.map((s) => (
                <button key={s.label} onClick={() => setPrompt(s.value)} className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${prompt === s.value ? 'bg-green-500 text-white border-green-500' : 'bg-white text-gray-600 border-gray-200 hover:border-green-300 hover:text-green-600'}`}>
                  {s.label}
                </button>
              ))}
            </div>
          ...
          <button onClick={generate} disabled={generating || !file || !prompt.trim()} className="w-full py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-semibold disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm shadow-green-500/30 flex items-center justify-center gap-2">
            {generating ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <label className="block text-sm font-semibold text-gray-800 mb-4">Generated Creative</label>
          {generating ? (
            <div className="aspect-square rounded-2xl border-2 border-dashed border-gray-100 bg-gray-50 flex flex-col items-center justify-center gap-3">
              <div className="w-8 h-8 border-[3px] border-green-500 border-t-transparent rounded-full animate-spin" />
              <p className="text-sm text-gray-500 font-medium">Generating creative…</p>
              <p className="text-xs text-gray-400">This takes about 30–60 seconds</p>
            </div>
          ) : images.length > 0 ? (
            <div className="flex justify-center">
              {images.map((url, i) => (
                <div key={i} className="group relative aspect-square w-full rounded-xl overflow-hidden border border-gray-100 bg-gray-50">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={url} alt="Generated creative" className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button onClick={() => useImage(url)} className="px-4 py-2 bg-green-500 text-white text-sm font-semibold rounded-xl hover:bg-green-600 transition-colors shadow-lg">
                      Use This
                    </button>
                  </div>
                </div>
            ...
            <div className="aspect-square rounded-2xl border-2 border-dashed border-gray-100 bg-gray-50 flex flex-col items-center justify-center gap-3 text-center px-8">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">Upload an image and describe the scene</p>
              </div>
            </div>
          )}
        </div>
