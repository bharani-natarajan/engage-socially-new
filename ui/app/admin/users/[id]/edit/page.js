  ...
  useEffect(() => {
    if (token && isAdmin && id) {
      adminApi.getUser(id)
        ...
        .catch((err) => setError(`Failed to load user: ${err.message}`))
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 py-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-lord-text-main">Edit User Account</h1>
          <p className="text-xs text-lord-text-muted mt-0.5">Modify settings and role for this user</p>
        </div>
      </div>
      ...
      {loading && <ActionLoader message="Saving user updates..." />}
    </div>
  );
}
